...
echo "BINARY_BUCKET_REGION: ${BINARY_BUCKET_REGION: -3}"
...
for binary in ${BINARIES[*]} ; do
    if [[ -n "$AWS_ACCESS_KEY_ID" ]]; then
        ...
        aws s3 cp --debug --region $BINARY_BUCKET_REGION $S3_PATH/$binary .
        aws s3 cp --debug --region $BINARY_BUCKET_REGION $S3_PATH/$binary.sha256 .
    else
